...
def addValue(table, values):
    ...
    query_raw2 = "INSERT INTO %s%s VALUES %s" % (table, attributes, inserts)
    ...
    try:
        ...
        for row in reader:
            ...
            if newrow == fakeUDict['Course']:
                ...
                # print('courseTableReached', courseTableReached)
            elif (newrow == fakeUDict['Student']) or (newrow == fakeUDict['Meeting']):
                courseTableReached = False
                # print('courseTableReached', courseTableReached)
            print(all_tuples)
            count = count + 1
